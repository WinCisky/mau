<script lang="ts">
  ...
  import { getLatestFollowedEpisodes } from "$lib/db_helper";
  ...
  onMount(async () => {
    getLatestFollowedEpisodes(pb).then((resultList) => {
      // console.log(JSON.stringify(resultList, null, 2));
      episodes = resultList.map((item) => {
  ...
  function loadMore() {
    getLatestFollowedEpisodes(pb, ++page).then((resultList) => {
